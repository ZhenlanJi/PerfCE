...
def main():
    with open(os.path.join("inference", 'noChaos_est.txt'), 'r') as fin:
    ...
    chaos_df = pd.read_csv(os.path.join(
        "output", 'chaos_info.csv'))
    chaos_attr = list(map(lambda x: "chaos__"+x, chaos_df.columns[1:]))

    model_save_path = "/home/zjiae/Results/exp_data/tidb_minmax"
